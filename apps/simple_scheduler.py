...
class SimpleScheduler(mesos.interface.Scheduler):
    ...
    def registered(self, driver, frameworkId, masterInfo):
        log("Registered with framework ID %s" % frameworkId.value)
    ...
    def resourceOffers(self, driver, offers):
        for offer in offers:
            ...
            log("Received offer %s with cpus: %s and mem: %s" % (offer.id.value, offerCpus, offerMem))
            ...
            while self.tasksLaunched < self.total_tasks and \
                ...
                log("Launching task %d using offer %s" % (tid, offer.id.value))
    ...
    def statusUpdate(self, driver, update):
        log("Task %s is in state %s" % (update.task_id.value, mesos_pb2.TaskState.Name(update.state)))
        ...
        if update.state == mesos_pb2.TASK_ERROR:
            self.tasksFinished += 1
            log("Error message: %s" % update.message)
        ...
        if update.state == mesos_pb2.TASK_LOST or \
            ...
            if self.retry_times <= self.max_retry_times:
                self.tasksLaunched -= 1
                self.retry_times += 1
                log("[retry %s] Task %s is in unexpected state %s with message '%s'" % (self.retry_times, update.task_id.value, mesos_pb2.TaskState.Name(update.state), update.message))
            else:
                log("Aborting because task %s is in unexpected state %s with message '%s'" % (update.task_id.value, mesos_pb2.TaskState.Name(update.state), update.message))
                driver.abort()
